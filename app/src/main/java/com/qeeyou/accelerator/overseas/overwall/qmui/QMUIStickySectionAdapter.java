...
public abstract class QMUIStickySectionAdapter<
    ...
    private void safeScrollToSection(@NonNull QMUISection<H, T> targetSection, boolean scrollToTop) {
        for (int i = 0; i < mSectionIndex.size(); i++) {
            ...
            if (itemIndex == QMUISection.ITEM_INDEX_SECTION_HEADER) {
    ...
    public int findPosition(PositionFinder<H, T> positionFinder, boolean unFoldTargetSection) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            ...
            int itemIndex = getItemIndex(i);
            if (itemIndex == QMUISection.ITEM_INDEX_SECTION_HEADER && targetItem == null) {
                return i;
            } else if (itemIndex >= 0) {
    ...
    public void toggleFold(int position, boolean scrollToTop) {
        ...
        if (scrollToTop && !section.isFold() && mViewCallback != null) {
            for (int i = 0; i < mSectionIndex.size(); i++) {
                ...
                if (itemIndex == QMUISection.ITEM_INDEX_SECTION_HEADER && getSection(pos) == section) {
    ...
    public final void onBindViewHolder(@NonNull final VH vh, int position) {
        ...
        } else if (itemIndex == QMUISection.ITEM_INDEX_LOAD_BEFORE || itemIndex == QMUISection.ITEM_INDEX_LOAD_AFTER) {
            onBindSectionLoadingItem(vh, position, section, itemIndex == QMUISection.ITEM_INDEX_LOAD_BEFORE);
        } else {
            onBindCustomItem(vh, position, section, itemIndex - QMUISection.ITEM_INDEX_CUSTOM_OFFSET);
        }
        if (itemIndex == QMUISection.ITEM_INDEX_LOAD_AFTER) {
            vh.isLoadBefore = false;
        } else if (itemIndex == QMUISection.ITEM_INDEX_LOAD_BEFORE) {
    ...
    public final int getItemViewType(int position) {
        ...
        if (itemIndex == QMUISection.ITEM_INDEX_SECTION_HEADER) {
            ...
            return ITEM_TYPE_CUSTOM_OFFSET + getCustomItemViewType(itemIndex - QMUISection.ITEM_INDEX_CUSTOM_OFFSET, position);
        }
    }
