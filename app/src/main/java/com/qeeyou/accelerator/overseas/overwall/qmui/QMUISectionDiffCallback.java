...
public class QMUISectionDiffCallback<H extends QMUISection.Model<H>, T extends QMUISection.Model<T>>
    ...
    private void generateIndex(List<QMUISection<H, T>> list,
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if(!removeSectionTitleIfOnlyOnceSection || list.size() > 1){
                generationInfo.appendIndex(i, QMUISection.ITEM_INDEX_SECTION_HEADER);
            }
            if (section.isFold()) {
                continue;
            }
            ...
            if (section.isExistBeforeDataToLoad()) {
                generationInfo.appendIndex(i, QMUISection.ITEM_INDEX_LOAD_BEFORE);
            }
            ...
            if (section.isExistAfterDataToLoad()) {
                generationInfo.appendIndex(i, QMUISection.ITEM_INDEX_LOAD_AFTER);
            }
    ...
    public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
        ...
        if (oldItemIndex == QMUISection.ITEM_INDEX_SECTION_HEADER) {
            return oldModel.isFold() == newModel.isFold() &&
                    oldModel.getHeader().isSameContent(newModel.getHeader());
        }

        if (oldItemIndex == QMUISection.ITEM_INDEX_LOAD_BEFORE || oldItemIndex == QMUISection.ITEM_INDEX_LOAD_AFTER) {
